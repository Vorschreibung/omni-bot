...
void G_RunMover( gentity_t *ent ) {
	...
	if ( ent->s.pos.trType != TR_STATIONARY || ent->s.apos.trType != TR_STATIONARY ) {
		// OSP - pausing
		if(level.match_pause == PAUSE_NONE) G_MoverTeam( ent );
			...
			case MOVER_POS1:
				...
				if (ent->allowteams) {
					...
					if (ent->allowteams & ALLOW_AXIS_TEAM) {
						...
						if (ent->allowteams & ALLOW_DISGUISED_CVOPS) blockflags |= AAS_AREA_TEAM_ALLIES_DISGUISED;
					}
					if (ent->allowteams & ALLOW_ALLIED_TEAM) {
						blockflags |= AAS_AREA_TEAM_ALLIES;
						if (ent->allowteams & ALLOW_DISGUISED_CVOPS) blockflags |= AAS_AREA_TEAM_AXIS_DISGUISED;
					}
					//
					if (!blockflags) {
						if (g_cheats.integer)
...
void SP_target_effect(gentity_t *ent)
{
	...
	if(G_SpawnString("type", "wood", &type)) {
		if (!Q_stricmp(type,"wood"))		ent->key = FXTYPE_WOOD;
		else if (!Q_stricmp(type,"glass"))	ent->key = FXTYPE_GLASS;
		else if (!Q_stricmp(type,"metal"))	ent->key = FXTYPE_METAL;
		else if (!Q_stricmp(type,"gibs"))	ent->key = FXTYPE_GIBS;
		else if (!Q_stricmp(type,"brick"))	ent->key = FXTYPE_BRICK;
		else if (!Q_stricmp(type,"rock"))	ent->key = FXTYPE_STONE;
		else if (!Q_stricmp(type,"fabric"))	ent->key = 0;	// fixme: not supported
	} else {
		...
		if( ent->s.angles2[0] < 5 ) {
			...
			if( ent->count2 ) {
				...
				if( ent->grenadeFired == ent->count2 )
					...
					case 1: G_Script_ScriptEvent( ent, "decayed", "stage1" ); break;
					case 2: G_Script_ScriptEvent( ent, "decayed", "stage2" ); break;
					case 3: G_Script_ScriptEvent( ent, "decayed", "stage3" ); break;
					}
				}
