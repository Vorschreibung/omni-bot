...
int BotFirstReachabilityArea( int entnum, vec3_t origin, int *areas, int numareas, qboolean distCheck )
{
	...
	for (i=0; i<numareas; i++) {
		if (botCheckedAreas[areas[i]]) continue;
...
int BotPointAreaNum(int entnum, vec3_t origin) {
	...
	if (ent && ent->client && ent->client->ps.pm_flags & PMF_LADDER) {
		...
		if (areanum && !trap_AAS_AreaLadder(areanum)) areanum = 0;
		if (areanum) {
			bestarea = areanum;
			goto done;
		}
		...
		if (numareas > 0) bestarea = BotFirstLadderArea(entnum, areas, numareas);
		if (bestarea) 
			goto done;

		// try the actual point
		areanum = trap_AAS_PointAreaNum(origin);
		if (areanum && !trap_AAS_AreaReachability(areanum)) areanum = 0;
		if (areanum) {
			bestarea = areanum;
			goto done;
		}
		...
		if (numareas > 0) bestarea = BotFirstReachabilityArea(entnum, origin, areas, numareas, qfalse);
		if (bestarea) 
			goto done;
		...
		VectorAdd( origin, maxs, maxs );
		numareas = trap_AAS_BBoxAreas(mins, maxs, areas, 50);
		if (numareas > 0) bestarea = BotFirstReachabilityArea(entnum, origin, areas, numareas, qtrue);
		if (bestarea) 
			goto done;
	}
...
qboolean BotFindNearbyTriggerGoal(bot_state_t *bs)
{
	...
	for (i = 0; goalnames[i]; i++)
	{
		...
		{
			if (BotHealthScale(bs->client) >= 1.0) continue;
		}
		//
		trav = NULL;
...
qboolean BotIsDead(bot_state_t *bs) {
	...
	if (bs->cur_ps.pm_type == PM_DEAD) return qtrue;
	if (g_entities[bs->client].health <= 0) return qtrue;
	return qfalse;
}
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	if (bs->cur_ps.groundEntityNum == ENTITYNUM_NONE) return moveresult;
	//initialize the movement state
	BotSetupForMovement(bs);
...
*/		//
		...
		if (trap_BotMoveInDirection(bs->ms, sideward, 190, movetype)) return moveresult;
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent, vec3_t entorigin) {
	...
	if (fov < 360 && !InFieldOfVision(viewangles, fov, entangles)) return 0;
...
qboolean BotVisibleFromPos( vec3_t srcorigin, int srcnum, vec3_t destorigin, int destent, qboolean dummy )
{
	...
	//
	if (BotEntityVisible( srcnum, eye, vec3_origin, 360, destent , destorigin)) return qtrue;
	//
	return qfalse;
}
...
qboolean BotCheckAttackAtPos(	int entnum, int enemy, vec3_t pos, qboolean ducking, qboolean allowHitWorld)
{
	...
	//
	if (BotEntityVisible( entnum, eye, vec3_origin, 360, enemy , NULL)) return qtrue;
	//
	return qfalse;
}
...
float BotGetMovementAutonomyRange( bot_state_t *bs, bot_goal_t *goal )
{
	...
	if (goal && goal->urgency >= BGU_MEDIUM) range = (range * (1.0 + 0.25*(goal->urgency - BGU_LOW)) ) + 256*(goal->urgency - BGU_LOW);
...
qboolean BotGoalWithinMovementAutonomy( bot_state_t *bs, bot_goal_t *goal, int urgency ) {
	...
	for (i=0, ignoreTrav=bs->ignoreGoals; i<MAX_IGNORE_GOALS; i++, ignoreTrav++) {
		if (!ignoreTrav->expireTime || ignoreTrav->expireTime <= level.time) continue;
		if (ignoreTrav->entityNum != goal->entitynum) continue;
		if (ignoreTrav->areanum != goal->areanum) continue;
...
qboolean BotCheckMovementAutonomy( bot_state_t *bs, bot_goal_t *goal )
{
	...
	if (goal->urgency == BGU_MAXIMUM) return qfalse;

	// are we outside range?
	if (!BotPointWithinMovementAutonomy( bs, goal, bs->origin )) return qtrue;

	// is our goal outside the range?
	if (!VectorCompare(vec3_origin, goal->origin) && !BotPointWithinMovementAutonomy( bs, goal, goal->origin )) return qtrue;
...
int BotFindEnemyMP(bot_state_t *bs, int curenemy, qboolean ignoreViewRestrictions) 
{
	...
	if (bs->last_findenemy == level.time) {
		...
		if (bot_profile.integer == 1) botTime_FindEnemy += trap_Milliseconds() - startTime;
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if (aim_accuracy > 1.0) aim_accuracy = 1.0;
...
qboolean BotCheckAttack(bot_state_t *bs) {
	...
	if (BotThrottleWeapon(bs->weaponnum)) {
		...
		if (bs->firethrottlewait_time > trap_AAS_Time()) return qtrue;
	...
	if (!InFieldOfVision(bs->viewangles, fov, angles)) return qtrue;
	...
	if (trace.ent != bs->enemy) {
		...
		if (trace.ent > 0 && trace.ent < MAX_CLIENTS) {
			...
			if (BotSameTeam(bs, trace.ent)) return qfalse;
		}
	}
	...
	if (wi.proj.damagetype & DAMAGETYPE_RADIAL) {
		...
		for (i=0; i<level.maxclients; i++) {
			...
			if (g_entities[i].client->sess.sessionTeam != TEAM_AXIS && g_entities[i].client->sess.sessionTeam != TEAM_ALLIES) continue;
			if (!CanDamage( BotGetEntity( i ), end ) && !CanDamage( BotGetEntity( i ), bs->origin )) continue;
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) 
{
	...
	if (bs->flags & BFL_DISMOUNT_MG42) {
		...
		if (!(g_entities[bs->client].s.eFlags & EF_MG42_ACTIVE)) {
			...
			if (rand()%2) trap_EA_Activate(bs->client);
		}
	}
...
void BotRecordPain( int client, int enemy, int mod ) {
	...
	if (g_entities[bs->client].health <= 0) return;
	...
	if (enemy >= level.maxclients) return;
	...
	if (bs->defendgoal.entitynum >= level.maxclients) {
		...
		if (VectorDistanceSquared( bs->origin, bs->defendgoal.origin ) > SQR(1024)) return;
...
int BotBestMG42Spot( bot_state_t *bs, qboolean force ) {
	...
	//while (trav = BotFindEntity( trav, FOFS(classname), "bot_mg42_spot" )) {
		...
		trav = mg42->melee;
		// is it disabled?
		if (mg42->aiInactive & (1<<bs->sess.sessionTeam)) continue;
		// if this is not for our team
		//if (trav->aiTeam && trav->aiTeam != bs->sess.sessionTeam) continue;
		if (!trav->melee->takedamage) continue;
		if (trav->melee->entstate != STATE_DEFAULT) continue;
		...
		if (!force && !mg42->active && trav->botIgnoreTime && (trav->botIgnoreTime > level.time - 1000)) continue;
		// get the travel time to the goal
		areanum = BotPointAreaNum( trav->s.number, trav->s.origin );
		if (!areanum) continue;
		t = trap_AAS_AreaTravelTimeToGoalArea( bs->areanum, bs->origin, areanum, bs->tfl );
...
//		newent = BotCheckBotGameEntity( newent );
		...
		while ((sptrav = G_Find( sptrav, FOFS(classname), "team_CTF_bluespawn" ))) {
			...
			VectorNormalize( v );
			if (DotProduct( v, dir ) > 0) blueCount++;
		}
		// RED
		sptrav = NULL;
		while ((sptrav = G_Find( sptrav, FOFS(classname), "team_CTF_redspawn" ))) {
			...
			VectorSubtract( sptrav->s.origin, trav->r.currentOrigin, v );
			VectorNormalize( v );
			if (DotProduct( v, dir ) > 0) redCount++;
		}
		// if the count is hugely in favor of one side, then it must belong to the other side
		if (blueCount - redCount > 4) newent->aiTeam = TEAM_AXIS;	// mostly facing blue spots
		else if (redCount - blueCount > 4) newent->aiTeam = TEAM_ALLIES;	// mostly facing red spots
...
qboolean BotCheckEmergencyTargets( bot_state_t *bs ) {
	...
	if (bot_profile.integer == 1) botTime_EmergencyGoals += trap_Milliseconds() - startTime;
...
qboolean BotFindSpecialGoals( bot_state_t *bs ) {
	...
	if (bot_profile.integer == 1) botTime_FindGoals += trap_Milliseconds() - startTime;
...
void BotShareLastAttacked( bot_state_t *bs )
{
	...
	gentity_t *ent, *trav;
	//
	if (bs->lastAttackShared > level.time - 400) return;
	...
	{
		if (!ptbs->inuse) continue;
		if (!BotSameTeam( bs, i )) continue;
		//
		tbs = ptbs;
		//
		if (ent->botLastAttackedTime >= trav->botLastAttackedTime) continue;	// no need to share
		if (VectorDistanceSquared( bs->origin, tbs->origin ) > SQR(2048)) continue;
		if (!trap_InPVS( bs->origin, tbs->origin )) continue;
		//
...
qboolean BotEnemyCarryingFlag( int entnum ) {
	...
	for (i=0, pbs=botstates; i<level.maxclients; i++, pbs++) {
		if (!pbs->inuse) continue;
		if (BotSameTeam( (pbs), entnum )) continue;
		if (pbs->sess.sessionTeam == TEAM_SPECTATOR) continue;
		if (!BotCarryingFlag( i )) continue;
		//
		// carrying flag
		return qtrue;
	}
...
{
	if (!g_entities[bs->client].botLastAttackedTime) return -99999;	// allow for game starting at time index 0
...
{
	if (!bs->last_pain) return -99999;	// allow for game starting at time index 0
	return bs->last_pain;
}
...
float *BotGetEye( int entnum )
{
	...
	//
	eye = &eyes[lastEye][0];
	if (++lastEye >= BOT_GETEYE_NUMEYES) lastEye=0;
	//
...
//	}
	//
	eye = &(eyes[lastEye][0]);
	if (++lastEye >= BOT_GETORIGIN_NUMEYES) lastEye=0;
	//
...
int BotTravelTimeToEntity( bot_state_t *bs, int entnum )
{
	int area = 0;
	//
	if (!bs->areanum) return 0;
	//
	area = BotGetArea( entnum );
	//
	if (!area) return 0;
	//
	if (!bs->tfl) bs->tfl = BotTravelFlagsForClient( bs->client );
...
qboolean BotDirectMoveToGoal( bot_state_t *bs, bot_goal_t *goal, bot_moveresult_t *moveresult )
{
	...
	float dist;
	//
	if (VectorDistanceSquared( bs->origin, goal->origin ) > SQR(1400)) return qfalse;
	if (!trap_InPVS( bs->origin, goal->origin )) return qfalse;
...
qboolean BotEntityTargetClassnameMatch( int entityNum, const char *classname )
{
	gentity_t *ent;
	//
	if (entityNum < 0 || entityNum > level.num_entities) return qfalse;
	ent = BotGetEntity( entityNum );
	if (!ent->inuse) return qfalse;
	if (!ent->target) return qfalse;
	if (!ent->target_ent) return qfalse;
	if (!ent->target_ent->inuse) return qfalse;
	//
	if (!Q_stricmp( ent->target_ent->classname, classname )) return qtrue;
	//
	return qfalse;
}
...
qboolean BotEntityWithinView( bot_state_t *bs, int viewEnt )
{
	...
	float	pitchMax=40.f;
	//
	if (viewEnt >= level.maxclients) return qfalse;
	if (level.clients[viewEnt].pers.connected != CON_CONNECTED) return qfalse;
	//
