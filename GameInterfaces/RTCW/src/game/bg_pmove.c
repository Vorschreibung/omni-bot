...
===============
*/
static void PM_BeginWeaponChange( int oldweapon, int newweapon ) {
	int switchtime;
...
static void PM_Weapon( void ) {
	...
	if ( pm->gametype >= GT_WOLF ) {
		...
		if ( pm->ps->weapon == WP_AMMO ) {
			charge = pm->ps->powerups[PW_NADEPACK] > 0 ? 0.5 : 0.25;

			if ( pm->cmd.serverTime - pm->ps->classWeaponTime < ( pm->ltChargeTime * charge ) ) {
				return;
			}
		}
		if ( pm->ps->weapon == WP_SMOKE_GRENADE ) {
			charge = pm->ps->powerups[PW_SMOKEGRENADE] > 0 ? 0.25 : 0.5;
	...
	if ( pm->waterlevel == 3 ) {
		if ( pm->ps->weapon != WP_KNIFE &&
			...
			int waterRevive = g_waterRevive.integer;
#elif CGAMEDLL
			extern vmCvar_t cg_waterRevive;
			int waterRevive = cg_waterRevive.integer;
#endif
			if ( ( pm->ps->weapon != WP_MEDIC_SYRINGE ) || waterRevive == 0 ) {
...
void PM_Sprint( void ) {
	...
	} else
	{
		...
		if ( pm->ps->powerups[PW_NOFATIGUE] ) { // (SA) recharge at 2x with stamina powerup
			pm->ps->sprintTime += 10 * multiplier;
		} else {
			pm->ps->sprintTime += 500 * pml.frametime * multiplier;     // JPW NERVE adjusted for framerate independence
